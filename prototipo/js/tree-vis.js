...
//      .on("zoom", zoom));
	...
	root = jerarquiaOrigen.hierarchy; //hierarchy[0] => root
	root.x0 = 0;
	root.y0 = 0;
...
function update(source) {
  ...
  nodes = jerarquiaOrigen.hierarchy; // tree.nodes(root);
...
function lineTopAndBottomHierarchy(links){
	...
	if(linkNivelTop.length == 0){
		//its root level
		var hierarchy_node = jerarquiaOrigen.getNodeByKey(root_key);
...
function upInTheHierarchy(element) {
	...
	var hierarchy_node = jerarquiaOrigen.getNodeByKey(element.id);
...
//	if(index_node != -1){
		var hierarchy_node = jerarquiaOrigen.getNodeByKey(node.key)
		if(hierarchy_node.level == "bottom")
...
function downInTheHierarchy(element){
	...
	let hierarchy_node = jerarquiaOrigen.getNodeByKey(element.id);
...
function nodeClick(d) {
	switch (d.level) {
		case "bottom":
				jerarquiaOrigen.changeNodeVisibility(d.key);
				if(jerarquiaOrigen.getVisibleBottomLevelNodes().length==0){
					jerarquiaOrigen.changeNodeVisibility(d.key);
				}
				callUpdate();
				break;
		case "top":
			if(d.depth !=0){//it is not root node
				jerarquiaOrigen.changeNodeVisibilityRecursive(d.key, !d.visible);
				if(jerarquiaOrigen.getVisibleTopLevelNodes().length==0){
					jerarquiaOrigen.changeNodeVisibilityRecursive(d.key, true);
				}
				callUpdate();
			}
			break;
...
function setNodeColor(){
	...
	.data(jerarquiaOrigen.hierarchy)
	.style({
		"fill":function(d){
					var nodeColor;
...
function updateHierarchy(bottom, top){
...
function callUpdate(){
	//AQUI CAMBIAR NIVEL BAJO Y ALTO
	jerarquiaOrigen.setBottomNodes(jerarquiaOrigen.getVisibleBottomLevelNodes());
	jerarquiaOrigen.setTopNodes(jerarquiaOrigen.getVisibleTopLevelNodes());
...
function treeVisMove(){
	...
	if(node_over_mouse!=null && node_over_mouse.level != "" && node_over_mouse.visible){
		...
		var father_disponible =  jerarquiaOrigen.getFatherDisponible(node_key);
		var children_disponible = jerarquiaOrigen.getChildrenDisponible(node_key);
...
function getNodeOverMouse(mouse_x, mouse_y){
	var length_limit = 40;
	for(var i = 0; i < jerarquiaOrigen.hierarchy.length;i++){
		var node = jerarquiaOrigen.hierarchy[i];
